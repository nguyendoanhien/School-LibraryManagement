...
namespace QuanLyThuVien
{
    public partial class frmPhieuMuon : Form
    {
        ...
        private void KT_DocGia_Click(object sender, EventArgs e)
        {
            ...
            if (txtMaDocGia.Text == "")
            {
                MessageBox.Show("Nh?p mã ??c gi?!!!");
                txtMaDocGia.Focus();
            }
            else if (docGia_BUS.Kt_DocGia(int.Parse(txtMaDocGia.Text)) == 1)
            {
                MessageBox.Show("Có th? m??n sách");
            }
            else if (docGia_BUS.Kt_DocGia(int.Parse(txtMaDocGia.Text)) == 0)
            {
                MessageBox.Show("?ây không ph?i là ??c gi?");
            }
            else
            { 
                MessageBox.Show("Không ???c m??n sách nha","C?nh báo",MessageBoxButtons.OK,MessageBoxIcon.Warning);
                txtMaDocGia.Text = "";
                txtMaDocGia.Focus();
            }
        }
        ...
        //        dgvSachMuon.Columns[1].HeaderText = "Mã ??u sách";
        //        dgvSachMuon.Columns[2].HeaderText = "Tên ??u sách";
        //        dgvSachMuon.Columns[3].HeaderText = "Ngày h?n tr?";
        //    }
        //}
        ...
        private void btnThemSach_Click(object sender, EventArgs e)
        {
            ...
            dgvSachMuon.Rows[index].Cells["Mã ??u sách"].Value = lblMaDauSach.Text;
        }
